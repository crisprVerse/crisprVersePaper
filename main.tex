...
\caption{\rev{\textbf{gRNA design functionalities implemented in \textit{crisprDesign} and commonly-used gRNA design tools.} Check marks indicate which functionalities are present in each tool at time of publication. $*$ In progress. $\dagger$ Information could not be found. See the Methods section for a detailed description of the criteria used for assessing feature availability.} }
\label{tab:methods}
\end{sidewaystable}
...
\multirow{11}{*}{SpCas9}   
                  ...
                  & \rev{WT} & \rev{RuleSet3} & \rev{On-target efficiency}  & \citet{ruleset3}  \\
                  ...
                  & \rev{WT} & \rev{DeepSpCas9} & \rev{On-target efficiency} & \citet{deepspcas9} \\
                  & \rev{WT} & \rev{CRISPRater} & \rev{On-target efficiency} & \citet{crisprater} \\
...
\rev{The function \textit{addCompositeScores} from \textit{crisprDesign} creates an aggregate score from a specified list of on-target scoring methods. It takes the average of the specified scores after performing a rank transformation. More specifically, consider $s_{ij}$ to be the score value for gRNA $i$ and method $j$. The composite score $S_{i}$ for gRNA $i$ is
...
\begin{equation}
\text{MIT} = \biggl(\prod_{p \in M}{c_p}\biggr)\times\frac{1}{\frac{\rev{L}-d}{\rev{L}}\times4+1}\times\frac{1}{m^2}
\end{equation}

where $M$ is the set of positions for which there is a mismatch between the gRNA spacer sequence and the off-target sequence, $c_p$ is an experimentally-derived mismatch tolerance weight at position $p$, $d$ is the average distance between mismatches, $m$ is the total number of mismatches, \rev{and $L$ is the spacer length}.  \rev{The spacer length used in \citet{mit} is 19.} As the number of mismatches increases, the cutting likelihood decreases. 
...
For \textit{CRISPick}, due to the lack of a CLI, we submitted batch query jobs through the portal \url{https://portals.broadinstitute.org/gppx/crispick/public} (accessed on July 27 2022) with default parameters for the Hsu (2013) tracrRNA sequence. 
}
...
\rev{A list of extensive tutorials can be found at \url{https://github.com/crisprVerse/Tutorials}.}
\rev{
...
\includegraphics[width=0.75\textwidth]{figures/speed/speedAcrossTools.pdf}
  \caption{\rev{\textbf{Comparison of computing times for subsets of human protein-coding exons.} 
...
\includegraphics[width=1\textwidth]{figures/cons/cons.pdf}
  \caption{\rev{\textbf{Influence of evolutionary conservation and gene target position on gRNA activity.} 
